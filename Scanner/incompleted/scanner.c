...
Token* getToken(void) {
  ...
  case CHAR_COLON: // Dau :
    ...
    if((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)){
      readChar();
      return makeToken(SB_ASSIGN, ln, cn); // Dau :=
    } else return makeToken(SB_COLON, ln, cn);
  case CHAR_SINGLEQUOTE: // Dau '
  ...
  case CHAR_LPAR: // Dau (
    ...
    case CHAR_PERIOD:
      readChar();
      return makeToken(SB_LSEL, ln, cn); // Dau (.
      break;
    case CHAR_TIMES:
      readChar();
      skipComment();
